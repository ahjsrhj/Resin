...
* ChainNodeHash：订阅级代理链节点 Hash。可为空；非空时，该订阅下节点的主动探测与真实转发都会先经过该节点，再由目标节点作为最终出口。
...
#### 订阅级代理链
* `ChainNodeHash` 只负责中间转发，不作为最终出口节点。
* 真实转发时，链路顺序为：`平台入口节点 -> 订阅级代理链节点 -> 目标节点`。缺失某一跳时自动退化为双跳或单跳。
* 主动探测时不使用 Platform 上下文，因此链路顺序为：`订阅级代理链节点 -> 目标节点`。
* 如果平台入口节点与订阅级代理链节点相同，只保留一份，不重复串联。
* 如果目标节点与任意前置 hop 相同，则该目标视为不可用，避免自环。
* 同一个节点可能被多个订阅持有。订阅级代理链归属规则为：
  * 优先已启用订阅；
  * 再按订阅 `CreatedAt` 最早优先；
...
* 探测动作：通过该节点的探测链路请求 `https://cloudflare.com/cdn-cgi/trace` (GET)。若订阅配置了 `ChainNodeHash`，则先经过该中间节点再由目标节点出网。
* 结果处理：
...
* 可选字段：`source_type`、`url`、`content`、`chain_node_hash`、`update_interval`、`enabled`、`ephemeral`、`ephemeral_node_evict_delay`
* 不可传字段：`id`、`node_count`、`healthy_node_count`、`created_at`、`last_checked`、`last_updated`、`last_error`
* 默认值：`update_interval="5m"`、`chain_node_hash=""`、`enabled=true`、`ephemeral=false`、`ephemeral_node_evict_delay="72h"`
...
* `chain_node_hash`：可为空；非空时必须是合法 Node Hash，且节点存在并且 `outbound` 已就绪。
...
* 必填字段：无
* 可改字段：`name`、`url`、`content`、`chain_node_hash`、`update_interval`、`enabled`、`ephemeral`、`ephemeral_node_evict_delay`
...
点击“新建”弹出订阅创建表单，包含名称、更新间隔、订阅来源（远程 URL / 本地内容）、代理链节点、临时订阅开关、临时节点驱逐延迟、启用开关。

点击表格行会打开订阅 Drawer，内含订阅配置编辑（基础信息、错误状态、代理链节点）与运维操作（手动刷新、清理失效节点、删除订阅）。
