...
* ChainPlatformID：订阅级代理链平台 ID。可为空；非空时，该订阅下节点的主动探测与真实转发都会先由该平台动态选择一个前置节点，再由目标节点作为最终出口。
...
#### 订阅级代理链
* `ChainPlatformID` 只负责为该订阅动态选择中间转发节点，不作为最终出口节点。
* 真实转发时，链路顺序为：`链平台动态选出的节点 -> 目标节点`。前置 hop 缺失时自动退化为单跳。
* 主动探测时沿用同一套链平台解析逻辑，因此链路顺序为：`链平台动态选出的节点 -> 目标节点`。
* 链平台选路始终忽略账号，只使用相同目标地址/域名参与路由决策。
* 链平台只贡献一个最终选中的节点，不展开其自身的入口节点或嵌套代理链。
* 如果目标节点与任意前置 hop 相同，则该目标视为不可用，避免自环。
* 同一个节点可能被多个订阅持有。订阅级代理链平台归属规则为：
  * 优先已启用订阅；
  * 再按订阅 `CreatedAt` 最早优先；
  * 同时间按 `subscription_id` 字典序最小优先；
  * 若没有任何已启用持有者，则回退到全部持有订阅按同样规则选择。
...
* 探测动作：通过该节点的探测链路请求 `https://cloudflare.com/cdn-cgi/trace` (GET)。若订阅配置了 `ChainPlatformID`，则先由该平台动态选一个中间节点再由目标节点出网。
* 结果处理：
...
* 探测动作：对全局配置的延迟探测站点沿该节点的探测链路发起 HTTP GET 请求，优先测量 **TLS Handshake** 耗时；若未产生 TLS 握手事件（如连接复用或明文 HTTP），回退为请求级 RTT（请求发起到首字节/请求完成）。
* 结果处理：
...
```json
{
  ...
  "source_type": "remote",
  "url": "https://example.com/sub",
  "chain_platform_id": "11111111-1111-1111-1111-111111111111",
  "update_interval": "5m",
  "enabled": true,
  "ephemeral": false,
  "ephemeral_node_evict_delay": "72h"
...
* 可选字段：`source_type`、`url`、`content`、`chain_platform_id`、`update_interval`、`enabled`、`ephemeral`、`ephemeral_node_evict_delay`
* 不可传字段：`id`、`node_count`、`healthy_node_count`、`created_at`、`last_checked`、`last_updated`、`last_error`
* 默认值：`update_interval="5m"`、`chain_platform_id=""`、`enabled=true`、`ephemeral=false`、`ephemeral_node_evict_delay="72h"`
...
* `chain_platform_id`：可为空；非空时必须是合法平台 UUID，且平台存在。
...
* 必填字段：无
* 可改字段：`name`、`url`、`content`、`chain_platform_id`、`update_interval`、`enabled`、`ephemeral`、`ephemeral_node_evict_delay`
...
点击“新建”弹出订阅创建表单，包含名称、更新间隔、订阅来源（远程 URL / 本地内容）、代理链平台、临时订阅开关、临时节点驱逐延迟、启用开关。

点击表格行会打开订阅 Drawer，内含订阅配置编辑（基础信息、错误状态、代理链平台）与运维操作（手动刷新、清理失效节点、删除订阅）。
